...
def generateSRAD(overflyRules, forbidden_entry_segs, forbidden_segs, points):
    ...
    for entry_node in forbidden_segs:
        srad.write("  (Point N (FL 0) (FL99000) "+entry_node+" "
    ...
    for entry_node in forbidden_segs:
        srad.write(
            "\n\n(Restriction \""+entry_node+"\" F \"Forbidden segments when entering in "
            +entry_node+"\" \"Generated by script\" \n()\n"
        )
        srad.write("(\n")
        for line in overflyRules[0:-1]:
            srad.write(line)
        srad.write("    (Point N (FL 0) (FL 99000) "+entry_node +" "
                +points[entry_node]['lon']+" "+points[entry_node]['lat']+")"
                )
        srad.write("\n  )\n)\n(\n")
        ...
        for seg in current_segs:
            srad.write("  (Segment N (FL 0) (FL99000) "+seg['airway']+" "
                +seg['from']+" "+points[seg['from']]['lon']+" "
                +points[seg['from']]['lat']+" "+seg['to']+" "
