...
def generateSRAD(overflyRules, forbidden_entry_segs, forbidden_segs, points):
    ...
    srad.write(
              "(SuluRad 1 0)\n"
              "(Restriction \"MANDATORY ENTRY POINTS\" M \"Mandatory entry points for China overflight\" \"Auto generated by script\" ()\n" 
              )
    for line in overflyRules:
        srad.write(line)
    srad.write( "\n(\n")
    for entry_node in forbidden_segs:
        srad.write("  (Point N (FL 0) (FL99000) "+entry_node+" "+points[entry_node]['lon']+" "+points[entry_node]['lat']+")\n")
    srad.write("))")
    ...
    for entry_node in forbidden_segs:
        ...
        for seg in current_segs:
            srad.write("   (Segment N (FL 0) (FL99000) "+seg['airway']+" "+seg['from']+" "+points[seg['from']]['lon'] +" "+points[seg['from']]['lat']+" "+seg['to']+" "+points[seg['to']]['lon'] + " " +points[seg['to']]['lat']+")\n")
        srad.write(")\n")
        srad.write(")")
    srad.close()
